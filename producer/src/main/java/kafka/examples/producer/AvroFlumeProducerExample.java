...
public class AvroFlumeProducerExample {
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            producerConfig.put("client.id", "avroflume-producer");
            ...
            SimpleProducer<String, byte[]> producer = new SimpleProducer<>(producerConfig, syncSend);

            for (int i = 0; i < noOfMessages; i++) {
                producer.send(topic, String.valueOf(i), getEvent(messageType, i));
                try {
                    Thread.sleep(delay);
    ...
    private static byte[] getEvent(String messageType, int i) {
        Map<CharSequence, CharSequence> eventHeader  = new HashMap<>();
        eventHeader.put("messageType", messageType);
        eventHeader.put("index", String.valueOf(i));
        AvroFlumeEvent avroFlumeEvent = AvroFlumeEvent.newBuilder()
                .setHeaders(eventHeader).setBody(ByteBuffer.wrap(String.format("%d: test message", i).getBytes())).build();
        return serialize(avroFlumeEvent);

    }

    private static byte[] serialize(AvroFlumeEvent event) {
        SpecificDatumWriter<AvroFlumeEvent> writer = new SpecificDatumWriter();
        ByteArrayOutputStream tempOutStream = new ByteArrayOutputStream();
        BinaryEncoder encoder = EncoderFactory.get().binaryEncoder(tempOutStream, null);
